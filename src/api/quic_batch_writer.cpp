...
public:
    ...
    void timeoutExpired() noexcept override {
        ...
        if (instance.socket_ && instance.batchWriter_ && !instance.batchWriter_->empty()) {
    ...
    quic::BatchWriter* FOLLY_NULLABLE getCachedWriter(quic::QuicBatchingMode mode,
    ...
    void setCachedWriter(quic::BatchWriter* writer) {
        if (enabled_) {
            ...
            if (evb && evb->getBackingEventBase() && !socket_) {
                ...
                if (fd >= 0) {
                    socket_ = std::make_unique<folly::AsyncUDPSocket>(evb->getBackingEventBase());
                    socket_->setFD(folly::NetworkSocket(fd), folly::AsyncUDPSocket::FDOwnership::OWNS);
                }
...
bool SinglePacketBatchWriter::append(std::unique_ptr<folly::IOBuf>&& buf,
    size_t /*unused*/, const folly::SocketAddress& /*unused*/, folly::AsyncUDPSocket* /*unused*/) {
...
ssize_t SinglePacketBatchWriter::write(folly::AsyncUDPSocket& sock, const folly::SocketAddress& address) {
...
bool GSOPacketBatchWriter::append(std::unique_ptr<folly::IOBuf>&& buf,
    size_t size, const folly::SocketAddress& /*unused*/, folly::AsyncUDPSocket* /*unused*/) {
...
GSOInplacePacketBatchWriter::GSOInplacePacketBatchWriter(
    QuicConnectionStateBase& conn, size_t maxPackets)
...
bool GSOInplacePacketBatchWriter::append(std::unique_ptr<folly::IOBuf>&& /*buf*/, size_t size,
    const folly::SocketAddress& /* addr */, folly::AsyncUDPSocket* /* sock */) {
 ...
 * conn_.bufAccessor.
 */
ssize_t GSOInplacePacketBatchWriter::write(folly::AsyncUDPSocket& sock, const folly::SocketAddress& address) {

    ScopedBufAccessor scopedBufAccessor(conn_.bufAccessor);
    //CHECK(lastPacketEnd_);
    auto& buf = scopedBufAccessor.buf();
    //CHECK(!buf->isChained());
    //CHECK(lastPacketEnd_ >= buf->data() && lastPacketEnd_ <= buf->tail()) << "lastPacketEnd_=" << (long)lastPacketEnd_ << " data=" << (long long)buf->data() << " tail=" << (long long)buf->tail();
    uint64_t diffToEnd = buf->tail() - lastPacketEnd_;
    //CHECK(diffToEnd <= conn_.udpSendPacketLen || (nextPacketSize_ && diffToEnd == nextPacketSize_)) << "diffToEnd=" << diffToEnd << ", pktLimit=" << conn_.udpSendPacketLen << ", nextPacketSize_=" << nextPacketSize_;
    if (diffToEnd >= conn_.udpSendPacketLen + kPacketSizeViolationTolerance) {
        //LOG(ERROR) << "Remaining buffer contents larger than udpSendPacketLen by " << (diffToEnd - conn_.udpSendPacketLen);
    }
    ...
    if (diffToEnd) {
        ...
        //CHECK_EQ(diffToEnd, bufLength) << "diffToEnd=" << diffToEnd << ", bufLength=" << bufLength;
        //CHECK(bufLength <= conn_.udpSendPacketLen || (nextPacketSize_ && bufLength == nextPacketSize_)) << "bufLength=" << bufLength << ", pktLimit=" << conn_.udpSendPacketLen << ", nextPacketSize_=" << nextPacketSize_;
...
SendmmsgPacketBatchWriter::SendmmsgPacketBatchWriter(size_t maxBufs) : maxBufs_(maxBufs) {
...
bool SendmmsgPacketBatchWriter::append(std::unique_ptr<folly::IOBuf>&& buf,
    size_t size, const folly::SocketAddress& /*unused*/, folly::AsyncUDPSocket* /*unused*/) {
...
ssize_t SendmmsgPacketBatchWriter::write(folly::AsyncUDPSocket& sock, const folly::SocketAddress& address) {
    //CHECK_GT(bufs_.size(), 0);
    if (bufs_.size() == 1) {
        return sock.write(address, bufs_[0]);
    }

    int ret = sock.writem(folly::range(&address, &address + 1), bufs_.data(), bufs_.size());
...
SendmmsgGSOPacketBatchWriter::SendmmsgGSOPacketBatchWriter(size_t maxBufs) : maxBufs_(maxBufs) {
    bufs_.reserve(maxBufs);
}
...
bool SendmmsgGSOPacketBatchWriter::append(std::unique_ptr<folly::IOBuf>&& buf,
    size_t size, const folly::SocketAddress& addr, folly::AsyncUDPSocket* sock) {
    ...
    if (idx.valid()) {
        if (size <= prevSize_[idx]) {
            if ((gso_[idx] == 0) || (static_cast<size_t>(gso_[idx]) == prevSize_[idx])) {
                // we can append
...
void BatchWriterDeleter::operator()(BatchWriter* batchWriter) {
    #if USE_THREAD_LOCAL_BATCH_WRITER
        ThreadLocalBatchWriterCache::getThreadLocalInstance().setCachedWriter(batchWriter);
    #else
        delete batchWriter;
    #endif
}
...
// BatchWriterFactory
BatchWriterPtr BatchWriterFactory::makeBatchWriter(const quic::QuicBatchingMode& batchingMode,
    uint32_t batchSize, bool useThreadLocal, const std::chrono::microseconds& threadLocalDelay,
    DataPathType dataPathType, QuicConnectionStateBase& conn, bool gsoSupported) {

#if USE_THREAD_LOCAL_BATCH_WRITER
    if (useThreadLocal && (batchingMode == quic::QuicBatchingMode::BATCHING_MODE_SENDMMSG_GSO) && gsoSupported) {
        BatchWriterPtr ret(ThreadLocalBatchWriterCache::getThreadLocalInstance().getCachedWriter(batchingMode, threadLocalDelay));
