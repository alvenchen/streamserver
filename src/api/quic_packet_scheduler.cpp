...
public:
    ...
    class MiddleStartingIterator : public boost::iterator_facade<MiddleStartingIterator,
    ...
    public:
        ...
        MiddleStartingIterator(const MapType* streams, const MapType::key_type& start)
            : streams_(streams) {
        ...
        MiddleStartingIterator(const MapType* streams, MapType::const_iterator itr) : streams_(streams), itr_(itr) {
            checkForWrapAround();
            // We don't want to mark it as wrapped around initially, instead just
            // act as if start was the first element.
            wrappedAround_ = false;
    ...
    MiddleStartingIterationWrapper(const MapType& streams, const MapType::key_type& start)
        : streams_(streams), start_(&streams_, start) {}

    MiddleStartingIterationWrapper(const MapType& streams, const MapType::const_iterator& start)
        : streams_(streams), start_(&streams_, start) {}
...
FrameScheduler::Builder::Builder(QuicConnectionStateBase& conn,
    EncryptionLevel encryptionLevel, PacketNumberSpace packetNumberSpace, folly::StringPiece name)
    : conn_(conn), encryptionLevel_(encryptionLevel),
      packetNumberSpace_(packetNumberSpace), name_(std::move(name)) {}
...
FrameScheduler FrameScheduler::Builder::build() && {
    ...
    if (ackScheduler_) {
        scheduler.ackScheduler_.emplace(AckScheduler(conn_, getAckState(conn_, packetNumberSpace_)));
    }
    if (rstScheduler_) {
    ...
    if (cryptoStreamScheduler_) {
        scheduler.cryptoStreamScheduler_.emplace(CryptoStreamScheduler(conn_, *getCryptoStream(*conn_.cryptoState, encryptionLevel_)));
    }
    ...
    if (immediateAckFrameScheduler_) {
        scheduler.immediateAckFrameScheduler_.emplace(ImmediateAckFrameScheduler(conn_));
    }
    return scheduler;
}

FrameScheduler::FrameScheduler(folly::StringPiece name, QuicConnectionStateBase& conn)
...
SchedulingResult FrameScheduler::scheduleFramesForPacket(PacketBuilderInterface&& builder, uint32_t writableBytes) {
    ...
    writableBytes = writableBytes > builder.getHeaderBytes() ? writableBytes - builder.getHeaderBytes() : 0;
    ...
    if (immediateAckFrameScheduler_ && immediateAckFrameScheduler_->hasPendingImmediateAckFrame()) {
        immediateAckFrameScheduler_->writeImmediateAckFrame(wrapper);
    }
    if (windowUpdateScheduler_ && windowUpdateScheduler_->hasPendingWindowUpdates()) {
    ...
    // limiting.
    if (simpleFrameScheduler_ && simpleFrameScheduler_->hasPendingSimpleFrames()) {
    ...
    if (datagramFrameScheduler_ && datagramFrameScheduler_->hasPendingDatagramFrames()) {
    ...
    if (builder.hasFramesPending()) {
        ...
        bool initialPacket = longHeader && longHeader->getHeaderType() == LongHeader::Types::Initial;
        if (initialPacket) {
...
bool StreamFrameScheduler::writeStreamLossBuffers(PacketBuilderInterface& builder, QuicStreamState& stream) {
    bool wroteStreamFrame = false;
    for (auto buffer = stream.lossBuffer.cbegin(); buffer != stream.lossBuffer.cend(); ++buffer) {
...
bool StreamFrameScheduler::writeSingleStream(PacketBuilderInterface& builder, QuicStreamState& stream,
...
StreamId StreamFrameScheduler::writeStreamsHelper(PacketBuilderInterface& builder,
    const std::set<StreamId>& writableStreams, StreamId nextScheduledStream,
    uint64_t& connWritableBytes, bool streamPerPacket) {
...
void StreamFrameScheduler::writeStreamsHelper(PacketBuilderInterface& builder,
    PriorityQueue& writableStreams, uint64_t& connWritableBytes, bool streamPerPacket) {
    ...
    do {
        ...
        //CHECK(stream) << "streamId=" << streamId << "inc=" << uint64_t(level.incremental);
        if (!writeSingleStream(builder, *stream, connWritableBytes)) {
            break;
        }
...
bool StreamFrameScheduler::writeStreamFrame(PacketBuilderInterface& builder,
    QuicStreamState& stream, uint64_t& connWritableBytes) {
    ...
    uint64_t flowControlLen = std::min(getSendStreamFlowControlBytesWire(stream), connWritableBytes);
...
AckScheduler::AckScheduler(const QuicConnectionStateBase& conn,const AckState& ackState)
    : conn_(conn), ackState_(ackState) {}

folly::Optional<PacketNum> AckScheduler::writeNextAcks(PacketBuilderInterface& builder) {
    ...
    uint8_t ackDelayExponentToUse = builder.getPacketHeader().getHeaderForm() == HeaderForm::Long
        ? kDefaultAckDelayExponent : conn_.transportSettings.ackDelayExponent;

    auto largestAckedPacketNum = *largestAckToSend(ackState_);
    auto ackingTime = Clock::now();
    //DCHECK(ackState_.largestRecvdPacketTime.hasValue()) << "Missing received time for the largest acked packet";
    ...
    std::chrono::microseconds ackDelay =
        (ackingTime > receivedTime ? std::chrono::duration_cast<std::chrono::microseconds>(ackingTime - receivedTime) : 0us);
    ...
    uint64_t peerRequestedTimestampsCount = conn_.maybePeerAckReceiveTimestampsConfig.has_value()
        ? conn_.maybePeerAckReceiveTimestampsConfig.value().maxReceiveTimestampsPerAck : 0;
    ...
    if (!isAckReceiveTimestampsSupported || !peerRequestedTimestampsCount) {
        ...
        ackWriteResult = writeAckFrameWithReceivedTimestamps(meta,
            builder, conn_.transportSettings.maybeAckReceiveTimestampsConfigSentToPeer.value(),
...
bool DatagramFrameScheduler::writeDatagramFrames(PacketBuilderInterface& builder) {
    bool sent = false;
    for (size_t i = 0; i <= conn_.datagramState.writeBuffer.size(); ++i) {
        ...
        auto datagramFrameLength = frameTypeQuicInt.getSize() + len + datagramLenInt.getSize();
...
WindowUpdateScheduler::WindowUpdateScheduler(const QuicConnectionStateBase& conn)
    : conn_(conn) {}

bool WindowUpdateScheduler::hasPendingWindowUpdates() const {
    return conn_.streamManager->hasWindowUpdates() || conn_.pendingEvents.connWindowUpdate;
}

void WindowUpdateScheduler::writeWindowUpdates(PacketBuilderInterface& builder) {
    ...
    for (const auto& windowUpdateStream : conn_.streamManager->windowUpdates()) {
        ...
        //VLOG(4) << "Wrote max_stream_data stream=" << stream->id << " maximumData=" << maximumData << " " << conn_;
    }
}
...
bool BlockedScheduler::hasPendingBlockedFrames() const {
    return !conn_.streamManager->blockedStreams().empty() || conn_.pendingEvents.sendDataBlocked;
}
...
void BlockedScheduler::writeBlockedFrames(PacketBuilderInterface& builder) {
    if (conn_.pendingEvents.sendDataBlocked) {
        ...
        DataBlockedFrame blockedFrame(conn_.flowControlState.peerAdvertisedMaxOffset);
...
CryptoStreamScheduler::CryptoStreamScheduler(const QuicConnectionStateBase& conn, const QuicCryptoStream& cryptoStream)
    : conn_(conn), cryptoStream_(cryptoStream) {}

bool CryptoStreamScheduler::writeCryptoData(PacketBuilderInterface& builder) {
    bool cryptoDataWritten = false;
    uint64_t writableData = folly::to<uint64_t>(cryptoStream_.writeBuffer.chainLength());
    ...
    for (const auto& buffer : cryptoStream_.lossBuffer) {
        ...
        //VLOG(4) << "Wrote retransmitted crypto" << " offset=" << buffer.offset << " bytes=" << res->len << " " << conn_;
    ...
    if (writableData != 0) {
        ...
        if (res) {
            //VLOG(4) << "Wrote crypto frame" << " offset=" << cryptoStream_.currentWriteOffset << " bytesWritten=" << res->len << " " << conn_;
            cryptoDataWritten = true;
        }
    }
    return cryptoDataWritten;
...
bool CryptoStreamScheduler::hasData() const {
    return !cryptoStream_.writeBuffer.empty() || !cryptoStream_.lossBuffer.empty();
}

ImmediateAckFrameScheduler::ImmediateAckFrameScheduler(const QuicConnectionStateBase& conn)
    : conn_(conn) {}
...
bool ImmediateAckFrameScheduler::writeImmediateAckFrame(PacketBuilderInterface& builder) {
    return 0 != writeFrame(ImmediateAckFrame(), builder);
}

CloningScheduler::CloningScheduler(FrameScheduler& scheduler, QuicConnectionStateBase& conn,
    const folly::StringPiece name, uint64_t cipherOverhead)
    : frameScheduler_(scheduler), conn_(conn), name_(name), cipherOverhead_(cipherOverhead) {
}

bool CloningScheduler::hasData() const {
    return frameScheduler_.hasData() || conn_.outstandings.numOutstanding() > conn_.outstandings.dsrCount;
}

SchedulingResult CloningScheduler::scheduleFramesForPacket(PacketBuilderInterface&& builder, uint32_t writableBytes) {
    ...
    if (conn_.version.has_value() && conn_.version.value() != QuicVersion::QUIC_V1) {
    ...
    // path if the write fails.
        return frameScheduler_.scheduleFramesForPacket(std::move(builder), writableBytes);
    }
    ...
    for (auto& outstandingPacket : conn_.outstandings.packets) {
        ...
        size_t prevSize = 0;
        if (conn_.transportSettings.dataPathType == DataPathType::ContinuousMemory) {
        ...
        if (conn_.transportSettings.dataPathType == DataPathType::ChainedMemory) {
            internalBuilder = std::make_unique<RegularQuicPacketBuilder>(
                conn_.udpSendPacketLen, header, getAckState(conn_, builderPnSpace).largestAckedByPeer.value_or(0));
        } else {
            //CHECK(conn_.bufAccessor && conn_.bufAccessor->ownsBuffer());
            internalBuilder = std::make_unique<InplaceQuicPacketBuilder>(*conn_.bufAccessor,
                conn_.udpSendPacketLen, header, getAckState(conn_, builderPnSpace).largestAckedByPeer.value_or(0));
        }
        // If the packet is already a clone that has been processed, we don't clone
        // it again.
        if (outstandingPacket.associatedEvent && conn_.outstandings.packetEvents.count(
        ...
        if (rebuildResult) {
            return SchedulingResult(std::move(rebuildResult), std::move(*internalBuilder).buildPacket());
        } else if (conn_.transportSettings.dataPathType == DataPathType::ContinuousMemory) {
