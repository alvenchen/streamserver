...
#include "BufAccessor.h"

namespace quic {
SimpleBufAccessor::SimpleBufAccessor(size_t capacity)
    : buf_(folly::IOBuf::createCombined(capacity)), capacity_(buf_->capacity()) {}
...
void SimpleBufAccessor::release(Buf buf) {
    ...
    //CHECK_EQ(buf->capacity(), capacity_) << "Buf has wrong capacity, capacit_=" << capacity_ << ", buf capacity=" << buf->capacity();
