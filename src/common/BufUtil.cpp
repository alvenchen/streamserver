...
void BufQueue::trimStart(size_t amount) {
    ...
    if (trimmed != amount) {
        throw std::underflow_error("Attempt to trim more bytes than are present in BufQueue");
    }
}
...
BufAppender::BufAppender(folly::IOBuf* data, size_t appendLen)
    : crtBuf_(data), head_(data), appendLen_(appendLen) {}
...
BufWriter::BufWriter(folly::IOBuf& iobuf, size_t most)
  ...
  //CHECK(iobuf_.tailroom() >= most_) << "Buffer room=" << iobuf_.tailroom() << " limit=" << most_;
}
