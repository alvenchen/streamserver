...
auto IntervalSet<T, Unit, Container>::intersectingRange(
    ...
    auto firstIt = std::lower_bound(container_type::begin(), container_type::end(), interval,
    ...
    auto endIt = firstIt;
    while (endIt != container_type::end() && endIt->start <= interval.end + interval_type::unitValue()) {
        ++endIt;
    }
