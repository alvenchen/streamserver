...
BbrCongestionController::BbrCongestionController(QuicConnectionStateBase& conn)
    : conn_(conn),
      ...
      initialCwnd_(conn.udpSendPacketLen * conn.transportSettings.initCwndInMss),
      recoveryWindow_(conn.udpSendPacketLen * conn.transportSettings.maxCwndInMss),
      pacingWindow_(conn.udpSendPacketLen * conn.transportSettings.initCwndInMss),
...
BbrCongestionController::BbrCongestionController(
    QuicConnectionStateBase& conn, uint64_t cwndBytes, std::chrono::microseconds minRtt)
    : conn_(conn), cwnd_(cwndBytes),
      initialCwnd_(conn.udpSendPacketLen * conn.transportSettings.initCwndInMss),
      recoveryWindow_(conn.udpSendPacketLen * conn.transportSettings.maxCwndInMss),
...
bool BbrCongestionController::updateRoundTripCounter(TimePoint largestAckedSentTime) noexcept {
...
void BbrCongestionController::setRttSampler(std::unique_ptr<MinRttSampler> sampler) noexcept {
    minRttSampler_ = std::move(sampler);
}

void BbrCongestionController::setBandwidthSampler(std::unique_ptr<BandwidthSampler> sampler) noexcept {
    bandwidthSampler_ = std::move(sampler);
}

void BbrCongestionController::onPacketLoss(const LossEvent& loss, uint64_t ackedBytes) {
    ...
    if (!inRecovery()) {
        ...
        recoveryWindow_ = boundedCwnd(recoveryWindow_, conn_.udpSendPacketLen, conn_.transportSettings.maxCwndInMss, kMinCwndInMssForBbr);
    ...
    recoveryWindow_ = recoveryWindow_ > loss.lostBytes + conn_.udpSendPacketLen * kMinCwndInMssForBbr
        ? recoveryWindow_ - loss.lostBytes : conn_.udpSendPacketLen * kMinCwndInMssForBbr;
...
void BbrCongestionController::onPacketSent(const OutstandingPacketWrapper& packet) {
...
uint64_t BbrCongestionController::updateAckAggregation(const AckEvent& ack) {
    ...
    uint64_t expectedAckBytes = bandwidth() * std::chrono::duration_cast<std::chrono::microseconds>(ack.ackTime - *ackAggregationStartTime_);
    ...
    aggregatedAckBytes_ += ack.ackedBytes;
    maxAckHeightFilter_.Update(aggregatedAckBytes_ - expectedAckBytes, roundTripCounter_);
    return aggregatedAckBytes_ - expectedAckBytes;
}

void BbrCongestionController::onPacketAckOrLoss(const AckEvent* FOLLY_NULLABLE ackEvent, const LossEvent* FOLLY_NULLABLE lossEvent) {
...
void BbrCongestionController::onPacketAcked(const AckEvent& ack, uint64_t prevInflightBytes, bool hasLoss) {
    SCOPE_EXIT {
        if (conn_.qLogger) {
            conn_.qLogger->addCongestionMetricUpdate(conn_.lossState.inflightBytes,
                getCongestionWindow(), kCongestionPacketAck, bbrStateToString(state_),
                bbrRecoveryStateToString(recoveryState_));
        }
    };
    if (ack.implicit) {
    ...
    if (ack.rttSample && minRttSampler_) {
        bool updated = minRttSampler_->newRttSample(ack.rttSample.value(), ack.ackTime);
        if (updated) {
            appLimitedSinceProbeRtt_ = false;
        }
    }

    bool newRoundTrip = updateRoundTripCounter(ack.largestNewlyAckedPacketSentTime);
    ...
    if (inRecovery()) {
        ...
        if (newRoundTrip && recoveryState_ != BbrCongestionController::RecoveryState::GROWTH) {
    ...
    const auto& ackFrequencyConfig = conn_.transportSettings.bbrConfig.ackFrequencyConfig;
    if (newRoundTrip && canSendAckControlFrames(conn_) && ackFrequencyConfig) {
        auto updatedMaxAckDelay = std::chrono::duration_cast<std::chrono::milliseconds>(clampMaxAckDelay(conn_, minRtt() / ackFrequencyConfig->minRttDivisor));
        // If we are either in STARTUP or haven't sent enough packets, based on
        // config.
        bool shouldUseInitThreshold =
            (ackFrequencyConfig->useSmallThresholdDuringStartup && state_ == BbrState::Startup) ||
            (!ackFrequencyConfig->useSmallThresholdDuringStartup &&
                (conn_.lossState.totalAckElicitingPacketsSent <= conn_.transportSettings.rxPacketsBeforeAckInitThreshold));
        // Default to 2 as the init threshold, which has been shown to be
        // a good choice empirically.
        uint32_t ackThreshold = shouldUseInitThreshold ? 2 : ackFrequencyConfig->ackElicitingThreshold;
        if ((!lastMaxAckDelay_ || lastMaxAckDelay_.value() != updatedMaxAckDelay) ||
            (!lastAckThreshold_ || lastAckThreshold_.value() != ackThreshold)) {
            
            requestPeerAckFrequencyChange(conn_, ackThreshold, updatedMaxAckDelay, ackFrequencyConfig->reorderingThreshold);
...
void BbrCongestionController::updatePacing() noexcept {
    ...
    if (state_ == BbrState::Startup) {
        conn_.pacer->setRttFactor(conn_.transportSettings.startupRttFactor.first, conn_.transportSettings.startupRttFactor.second);
    } else {
        conn_.pacer->setRttFactor(conn_.transportSettings.defaultRttFactor.first, conn_.transportSettings.defaultRttFactor.second);
    }
...
void BbrCongestionController::handleAckInProbeBw(TimePoint ackTime, uint64_t prevInflightBytes, bool hasLoss) noexcept {
    bool shouldAdvancePacingGainCycle = ackTime - cycleStart_ > minRtt();
    if (pacingGain_ > 1.0 && !hasLoss && prevInflightBytes < calculateTargetCwnd(pacingGain_)) {
    ...
    if (shouldAdvancePacingGainCycle) {
        ...
        if (!isInBackgroundMode() &&
            ...
            auto drainTarget = targetCwndCache ? *targetCwndCache : calculateTargetCwnd(1.0);
...
bool BbrCongestionController::shouldExitDrain() noexcept {
    return state_ == BbrState::Drain && conn_.lossState.inflightBytes <= calculateTargetCwnd(1.0);
}

bool BbrCongestionController::shouldProbeRtt(TimePoint ackTime) noexcept {
    if (conn_.transportSettings.bbrConfig.probeRttDisabledIfAppLimited && appLimitedSinceProbeRtt_) {
    ...
    // ProbeRtt.
    if (state_ != BbrState::ProbeRtt && minRttSampler_ && !exitingQuiescene_ && minRttSampler_->minRttExpired()) {
...
void BbrCongestionController::handleAckInProbeRtt(bool newRoundTrip, TimePoint ackTime) noexcept {
    ...
    if (!earliestTimeToExitProbeRtt_ && conn_.lossState.inflightBytes < getCongestionWindow() + conn_.udpSendPacketLen) {
...
size_t BbrCongestionController::pickRandomCycle() {
    pacingCycleIndex_ = (folly::Random::rand32(numOfCycles_ - 1) + 2) % numOfCycles_;
    //DCHECK_NE(pacingCycleIndex_, 1);
    return pacingCycleIndex_;
}

void BbrCongestionController::updateRecoveryWindowWithAck(uint64_t bytesAcked) noexcept {
    //DCHECK(inRecovery());
    if (recoveryState_ == BbrCongestionController::RecoveryState::GROWTH) {
        recoveryWindow_ += bytesAcked;
    }

    uint64_t recoveryIncrease = conn_.transportSettings.bbrConfig.conservativeRecovery
        ? conn_.udpSendPacketLen : bytesAcked;
    recoveryWindow_ = std::max(recoveryWindow_, conn_.lossState.inflightBytes + recoveryIncrease);
    recoveryWindow_ = boundedCwnd(recoveryWindow_, conn_.udpSendPacketLen, conn_.transportSettings.maxCwndInMss, kMinCwndInMssForBbr);
}
...
BbrCongestionController::BbrState BbrCongestionController::state() const noexcept {
    return state_;
}
...
uint64_t BbrCongestionController::calculateTargetCwnd(float gain) const noexcept {
...
void BbrCongestionController::updateCwnd(uint64_t ackedBytes, uint64_t excessiveBytes) noexcept {
    ...
    } else if (cwnd_ < targetCwnd || conn_.lossState.totalBytesAcked < initialCwnd_) {
        // This is a bit strange. The argument here is that if we haven't finished
        // STARTUP, then forget about the gain calculation.
        cwnd_ += ackedBytes;
    }

    cwnd_ = boundedCwnd(cwnd_, conn_.udpSendPacketLen, conn_.transportSettings.maxCwndInMss, kMinCwndInMssForBbr);
}
...
void BbrCongestionController::setBandwidthUtilizationFactor(float bandwidthUtilizationFactor) noexcept {
    ...
    if (bandwidthUtilizationFactor_ < 1.0) {
        ...
        pacingGainCycles_.clear();
        pacingGainCycles_.assign(kPacingGainCycles.begin(), kPacingGainCycles.end());
        if (state_ == BbrState::Startup) {
            pacingGain_ = kStartupGain;
            cwndGain_ = kStartupGain;
        }
...
folly::Optional<Bandwidth> BbrCongestionController::getBandwidth() const noexcept {
    if (bandwidthSampler_) {
...
void BbrCongestionController::onRemoveBytesFromInflight(uint64_t bytesToRemove) {
...
std::string bbrRecoveryStateToString(BbrCongestionController::RecoveryState recoveryState) {
