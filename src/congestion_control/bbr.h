...
public:
    ...
    class BandwidthSampler {
        ...
        virtual void onPacketAcked(const CongestionController::AckEvent&, uint64_t roundTripCounter) = 0;
    ...
    explicit BbrCongestionController(QuicConnectionStateBase& conn, uint64_t cwndBytes, std::chrono::microseconds minRtt);
    ...
    void onPacketAckOrLoss(const AckEvent* FOLLY_NULLABLE, const LossEvent* FOLLY_NULLABLE) override;
    void onPacketAckOrLoss(folly::Optional<AckEvent> ack, folly::Optional<LossEvent> loss) {
    ...
    [[nodiscard]] folly::Optional<Bandwidth> getBandwidth() const noexcept override;
    ...
    */
    void setBandwidthUtilizationFactor(float bandwidthUtilizationFactor) noexcept override;
...
private:
    ...
    */
    void onPacketAcked(const AckEvent& ack, uint64_t prevInflightBytes, bool hasLoss);
     ...
     */
    void handleAckInProbeBw(TimePoint ackTime, uint64_t prevInflightBytes, bool hasLoss) noexcept;
    ...
    */
    ...
    WindowedFilter<uint64_t /* ack bytes count */, 
        MaxFilter<uint64_t>, uint64_t /* roundtrip count */, 
        uint64_t /* roundtrip count */> maxAckHeightFilter_;
    ...
    friend std::ostream& operator<<(std::ostream& os, const BbrCongestionController& bbr);
};
