...
Bbr2CongestionController::Bbr2CongestionController(QuicConnectionStateBase& conn)
    : conn_(conn), maxBwFilter_(kMaxBwFilterLen, Bandwidth(), 0),
...
void Bbr2CongestionController::onRemoveBytesFromInflight(uint64_t bytesToRemove) {
    subtractAndCheckUnderflow(conn_.lossState.inflightBytes, bytesToRemove);
}

void Bbr2CongestionController::onPacketSent(const OutstandingPacketWrapper& packet) {
...
void Bbr2CongestionController::onPacketAckOrLoss(const AckEvent* FOLLY_NULLABLE ackEvent, const LossEvent* FOLLY_NULLABLE lossEvent) {
/*
    if (conn_.qLogger) {
        conn_.qLogger->addCongestionMetricUpdate(conn_.lossState.inflightBytes, getCongestionWindow(),
            kCongestionPacketAck, bbr2StateToString(state_));
    }
*/
    if (ackEvent) {
        subtractAndCheckUnderflow(conn_.lossState.inflightBytes, ackEvent->ackedBytes);
    }
    if (lossEvent) {
        subtractAndCheckUnderflow(conn_.lossState.inflightBytes, lossEvent->lostBytes);
    }
    SCOPE_EXIT {
        //VLOG(6) << fmt::format("State={} inflight={} cwnd={} (gain={})", bbr2StateToString(state_), conn_.lossState.inflightBytes, getCongestionWindow(), cwndGain_);
    };
    ...
    if (lossEvent && lossEvent->lostPackets > 0) {
        ...
        if (lossEvent->persistentCongestion) {
            ...
            cwndBytes_ = conn_.lossState.inflightBytes + std::max(newlyAcked, conn_.udpSendPacketLen);
        }
    }

    if (ackEvent) {
        if (appLimited_ && appLimitedLastSendTime_ <= ackEvent->largestNewlyAckedPacketSentTime) {
            appLimited_ = false;
        }

        if (inPacketConservation_ &&
            packetConservationStartTime_ <= ackEvent->largestNewlyAckedPacketSentTime) {
        ...
        auto inflightBytesAtLargestAckedPacket = ackEvent->getLargestNewlyAckedPacket()
            ? ackEvent->getLargestNewlyAckedPacket()->outstandingPacketMetadata.inflightBytes
            : conn_.lossState.inflightBytes;
        auto lostBytes = lossEvent ? lossEvent->lostBytes : 0;
        updateProbeBwCyclePhase(ackEvent->ackedBytes, inflightBytesAtLargestAckedPacket, lostBytes);
        updateMinRtt();
...
uint64_t Bbr2CongestionController::getWritableBytes() const noexcept {
    return getCongestionWindow() > conn_.lossState.inflightBytes
        ? getCongestionWindow() - conn_.lossState.inflightBytes : 0;
}
...
void Bbr2CongestionController::setPacing() {
    ...
    //VLOG(6) << fmt::format("Setting pacing to {}. From bandwdith_={} pacingGain_={} kPacingMarginPercent={} units={} interval={}", rate.normalizedDescribe(), bandwidth_.normalizedDescribe(), pacingGain_, kPacingMarginPercent, rate.units, rate.interval.count());
...
void Bbr2CongestionController::setCwnd(uint64_t ackedBytes, uint64_t lostBytes) {
    // BBRUpdateMaxInflight()
    inflightMax_ = addQuantizationBudget(getBDPWithGain(cwndGain_) + maxExtraAckedFilter_.GetBest());
    // BBRModulateCwndForRecovery()
    if (lostBytes > 0) {
        cwndBytes_ = std::max(cwndBytes_ - std::min(lostBytes, cwndBytes_), kMinCwndInMssForBbr * conn_.udpSendPacketLen);
    }
    if (inPacketConservation_) {
        cwndBytes_ = std::max(cwndBytes_, conn_.lossState.inflightBytes + ackedBytes);
    }

    if (!inPacketConservation_) {
        if (filledPipe_) {
            cwndBytes_ = std::min(cwndBytes_ + ackedBytes, inflightMax_);
        } else if (cwndBytes_ < inflightMax_ || 
                conn_.lossState.totalBytesAcked < conn_.transportSettings.initCwndInMss * conn_.udpSendPacketLen) {
            cwndBytes_ += ackedBytes;
        }
        cwndBytes_ = std::max(cwndBytes_, kMinCwndInMssForBbr * conn_.udpSendPacketLen);
    }
...
void Bbr2CongestionController::updateLatestDeliverySignals(const AckEvent& ackEvent) {
    lossRoundStart_ = false;

    bandwidthLatest_ = std::max(bandwidthLatest_, getBandwidthSampleFromAck(ackEvent));
    //VLOG(6) << fmt::format("Bandwidth latest= {}  AppLimited={}", bandwidthLatest_.normalizedDescribe(), bandwidthLatest_.isAppLimited);
    inflightLatest_ = std::max(inflightLatest_, bandwidthLatest_.units);

    auto pkt = ackEvent.getLargestNewlyAckedPacket();
    if (pkt && pkt->outstandingPacketMetadata.totalBytesSent > lossRoundEndBytesSent_) {
        ...
        lossPctInLastRound_ = static_cast<float>(lossBytesInRound_) /
            static_cast<float>(conn_.lossState.totalBytesSent - lossRoundEndBytesSent_);
...
void Bbr2CongestionController::updateCongestionSignals(const LossEvent* FOLLY_NULLABLE lossEvent) {
    // Update max bandwidth
    if (bandwidthLatest_ > maxBwFilter_.GetBest() || !bandwidthLatest_.isAppLimited) {
        //VLOG(6) << fmt::format("Updating bandwidth filter with sample: {}", bandwidthLatest_.normalizedDescribe());
    ...
    if (lossBytesInRound_ > 0) {
        ...
        inflightLo_ = std::max(inflightLatest_, static_cast<uint64_t>(inflightLo_ * kBeta));
    }
...
void Bbr2CongestionController::updateAckAggregation(const AckEvent& ackEvent) {
    ...
    auto interval = Clock::now() - extraAckedStartTimestamp_.value_or(TimePoint());
    auto expectedDelivered = bandwidth_ * std::chrono::duration_cast<std::chrono::microseconds>(interval);
...
void Bbr2CongestionController::checkStartupFullBandwidth() {
    ...
    if (maxBwFilter_.GetBest() >= filledPipeBandwidth_ * 1.25) { /* still growing? */
    filledPipeBandwidth_ = maxBwFilter_.GetBest(); /* record new baseline level */
    filledPipeCount_ = 0;
        return;
    }
...
void Bbr2CongestionController::checkDrain() {
    if (state_ == State::Drain) {
        //VLOG(6) << fmt::format("Current inflight {} target inflight {}", conn_.lossState.inflightBytes, getTargetInflightWithGain(1.0));
    }
    if (state_ == State::Drain &&
...
void Bbr2CongestionController::updateProbeBwCyclePhase(uint64_t ackedBytes, 
    uint64_t inflightBytesAtLargestAckedPacket, uint64_t lostBytes) {
    ...
    switch (state_) {
        ...
        case State::ProbeBw_Up:
            if (hasElapsedInPhase(minRtt_) && inflightLatest_ > getTargetInflightWithGain(1.25)) {
                startProbeBwDown();
            }
            break;
        default:
            throw QuicInternalException(
                "BBR2: Unexpected state in ProbeBW phase: " +
                    bbr2StateToString(state_), LocalErrorCode::CONGESTION_CONTROL_ERROR);
    }
}

void Bbr2CongestionController::adaptUpperBounds(uint64_t ackedBytes,
    uint64_t inflightBytesAtLargestAckedPacket, uint64_t lostBytes) {
...
bool Bbr2CongestionController::hasElapsedInPhase(std::chrono::microseconds interval) {
    return Clock::now() > probeBWCycleStart_ + interval;
}

// Was the loss percent too hight for the last ack received?
bool Bbr2CongestionController::checkInflightTooHigh(uint64_t inflightBytesAtLargestAckedPacket, uint64_t lostBytes) {
...
bool Bbr2CongestionController::isInflightTooHigh(uint64_t inflightBytesAtLargestAckedPacket, uint64_t lostBytes) {
    return static_cast<float>(lostBytes) > static_cast<float>(inflightBytesAtLargestAckedPacket) * kLossThreshold;
}

void Bbr2CongestionController::handleInFlightTooHigh(uint64_t inflightBytesAtLargestAckedPacket) {
    bwProbeSamples_ = 0;
    ...
    if (!isAppLimited()) {
        ...
        static_cast<uint64_t>(static_cast<float>(getTargetInflightWithGain()) * kBeta));
    }
    if (state_ == State::ProbeBw_Up) {
        startProbeBwDown();
    }
...
uint64_t Bbr2CongestionController::getTargetInflightWithHeadroom() const {
    ...
    * RTTs and loss */
    ...
    auto headroom = static_cast<uint64_t>(std::max(1.0f, kHeadroomFactor * static_cast<float>(inflightHi_)));
    return std::max(inflightHi_ - headroom, quic::kMinCwndInMssForBbr * conn_.udpSendPacketLen);
}
...
void Bbr2CongestionController::updateMinRtt() {
    probeRttExpired_ = probeRttMinTimestamp_ ? Clock::now() > (probeRttMinTimestamp_.value() + kProbeRTTInterval) : true;
    ...
    auto minRttExpired = minRttTimestamp_
        ? Clock::now() > (minRttTimestamp_.value() + kMinRttFilterLen) : true;
...
void Bbr2CongestionController::handleProbeRtt() {
    ...
    if (!probeRttDoneTimestamp_ && conn_.lossState.inflightBytes <= getProbeRTTCwnd()) {
...
void Bbr2CongestionController::advanceLatestDeliverySignals(const AckEvent& ackEvent) {
    if (lossRoundStart_) {
...
uint64_t Bbr2CongestionController::getProbeRTTCwnd() {
    return std::max(getBDPWithGain(kProbeRttCwndGain), quic::kMinCwndInMssForBbr * conn_.udpSendPacketLen);
}
...
void Bbr2CongestionController::boundBwForModel() {
    if (state_ == State::Startup) {
        ...
        bandwidth_ = std::min(std::min(maxBwFilter_.GetBest(), bandwidthLo_), bandwidthHi_);
    }
    /*
    if (conn_.qLogger) {
...
uint64_t Bbr2CongestionController::getBDPWithGain(float gain) const {
    if (minRtt_ == kDefaultMinRtt) {
        return uint64_t(gain * conn_.transportSettings.initCwndInMss * conn_.udpSendPacketLen);
    } else {
...
void Bbr2CongestionController::startProbeBwDown() {
    resetCongestionSignals();
    probeUpCount_ = std::numeric_limits<uint64_t>::max(); /* not growing inflight_hi */
...
void Bbr2CongestionController::raiseInflightHiSlope() {
    ...
    probeUpCount_ = std::max(cwndBytes_ / growthThisRound, decltype(cwndBytes_)(1));
}

// Utilities
bool Bbr2CongestionController::isProbeBwState(const Bbr2CongestionController::State state) {
    return (
...
Bandwidth Bbr2CongestionController::getBandwidthSampleFromAck(const AckEvent& ackEvent) {
    ...
    auto lastSentTime = lastAckedPacket ? lastAckedPacket->sentTime : conn_.connectionTime;

    auto sendElapsed = pkt->outstandingPacketMetadata.time - lastSentTime;

    auto lastAckTime = lastAckedPacket ? lastAckedPacket->adjustedAckTime : conn_.connectionTime;
    auto ackElapsed = ackTime - lastAckTime;
    auto interval = std::max(ackElapsed, sendElapsed);
    if (interval == 0us) {
        return Bandwidth();
    }
    auto lastBytesDelivered = lastAckedPacket ? lastAckedPacket->totalBytesAcked : 0;
    auto bytesDelivered = ackEvent.totalBytesAcked - lastBytesDelivered;
    Bandwidth bwSample(bytesDelivered,
...
bool Bbr2CongestionController::isRenoCoexistenceProbeTime() {
    auto renoBdpInPackets = std::min(getTargetInflightWithGain(), cwndBytes_) / conn_.udpSendPacketLen;
    auto roundsBeforeRenoProbe = std::min(renoBdpInPackets, decltype(renoBdpInPackets)(63));
    return roundsSinceBwProbe_ >= roundsBeforeRenoProbe;
}

Bbr2CongestionController::State Bbr2CongestionController::getState()const noexcept {
    return state_;
}

void Bbr2CongestionController::getStats(CongestionControllerStats& stats) const {
