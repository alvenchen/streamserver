...
BbrBandwidthSampler::BbrBandwidthSampler(QuicConnectionStateBase& conn)
    : conn_(conn), windowedFilter_(bandwidthWindowLength(kNumOfCycles), Bandwidth(), 0) {}
...
void BbrBandwidthSampler::setWindowLength(const uint64_t windowLength) noexcept {
    windowedFilter_.SetWindowLength(windowLength);
}

void BbrBandwidthSampler::onPacketAcked(const CongestionController::AckEvent& ackEvent, uint64_t rttCounter) {
    if (appLimited_) {
    ...
    for (auto const& ackedPacket : ackEvent.ackedPackets) {
        ...
        if (ackedPacket.lastAckedPacketInfo) {
            //DCHECK(ackedPacket.outstandingPacketMetadata.time > ackedPacket.lastAckedPacketInfo->sentTime);
            //DCHECK_GE(ackedPacket.outstandingPacketMetadata.totalBytesSent, ackedPacket.lastAckedPacketInfo->totalBytesSent);
            sendRate = Bandwidth(ackedPacket.outstandingPacketMetadata.totalBytesSent -
                    ackedPacket.lastAckedPacketInfo->totalBytesSent,
                    std::chrono::duration_cast<std::chrono::microseconds>(ackedPacket.outstandingPacketMetadata.time - ackedPacket.lastAckedPacketInfo->sentTime));

            //DCHECK(ackEvent.ackTime > ackedPacket.lastAckedPacketInfo->ackTime);
            //DCHECK_GE(conn_.lossState.totalBytesAcked, ackedPacket.lastAckedPacketInfo->totalBytesAcked);
            auto ackDuration = (ackEvent.adjustedAckTime > ackedPacket.lastAckedPacketInfo->adjustedAckTime)
                ? (ackEvent.adjustedAckTime - ackedPacket.lastAckedPacketInfo->adjustedAckTime)
                : (ackEvent.ackTime - ackedPacket.lastAckedPacketInfo->ackTime);
            ackRate = Bandwidth(conn_.lossState.totalBytesAcked - ackedPacket.lastAckedPacketInfo->totalBytesAcked,
            ...
            sendRate = Bandwidth(ackEvent.ackedBytes, std::chrono::duration_cast<std::chrono::microseconds>(
        ...
        if (measuredBandwidth >= windowedFilter_.GetBest() || !ackedPacket.isAppLimited) {
