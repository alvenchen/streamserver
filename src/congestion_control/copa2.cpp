...
Copa2::Copa2(QuicConnectionStateBase& conn)
    : conn_(conn), cwndBytes_(conn.transportSettings.initCwndInMss * conn.udpSendPacketLen),
      minRTTFilter_(kCopa2MinRttWindowLength.count(), 0us, 0) {

    //VLOG(10) << __func__ << " writable=" << Copa2::getWritableBytes() << " cwnd=" << cwndBytes_ << " inflight=" << conn_.lossState.inflightBytes << " " << conn_;
}
...
void Copa2::onPacketAckOrLoss(const AckEvent* FOLLY_NULLABLE ack, const LossEvent* FOLLY_NULLABLE loss) {
    if (loss) {
        onPacketLoss(*loss);
        if (conn_.pacer) {
...
void Copa2::manageLossyMode(folly::Optional<TimePoint> sentTime) {
    ...
    //VLOG(5) << __func__ << " lossyMode=" << lossyMode_ << " num lost=" << numLostInLossCycle_ << " num acked=" << numAckedInLossCycle_ << " " << conn_;
...
void Copa2::onPacketLoss(const LossEvent& loss) {
    ...
    if (conn_.qLogger) {
        conn_.qLogger->addCongestionMetricUpdate(conn_.lossState.inflightBytes,
            getCongestionWindow(), kCongestionPacketLoss);
    }
    */
    ...
    if (loss.persistentCongestion) {
        ...
        if (conn_.qLogger) {
            conn_.qLogger->addCongestionMetricUpdate(conn_.lossState.inflightBytes,
                getCongestionWindow(), kPersistentCongestion);
        }
        */
    }
...
void Copa2::onPacketAcked(const AckEvent& ack) {
    ...
    //VLOG(5) << __func__ << "updated cwnd=" << cwndBytes_ << " rttMin=" << rttMin.count() << " lrtt=" << conn_.lossState.lrtt.count() << " dParam=" << dParam.count() << " " << conn_;
