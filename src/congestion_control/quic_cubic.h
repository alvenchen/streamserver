...
public:
    static constexpr uint64_t INIT_SSTHRESH = std::numeric_limits<uint64_t>::max();
    /**
     ...
     */
    explicit Cubic(QuicConnectionStateBase& conn, uint64_t initCwndBytes = 0,
        uint64_t initSsthresh = INIT_SSTHRESH, bool tcpFriendly = true, bool ackTrain = false);
    ...
    void handoff(uint64_t newCwnd, uint64_t ssthresh = INIT_SSTHRESH, TimePoint lastReductionTime = Clock::now()) noexcept;
