...
void StaticCwndCongestionController::onRemoveBytesFromInflight(uint64_t bytesToRemove) {
    subtractAndCheckUnderflow(inflightBytes_, bytesToRemove);
}

void StaticCwndCongestionController::onPacketSent(const OutstandingPacketWrapper& packet) {
    isAppLimited_ = false;
    addAndCheckOverflow(inflightBytes_, packet.metadata.encodedSize);
}

void StaticCwndCongestionController::onPacketAckOrLoss(const AckEvent* FOLLY_NULLABLE ackEvent, const LossEvent* FOLLY_NULLABLE lossEvent) {
    if (ackEvent) {
...
uint64_t StaticCwndCongestionController::getWritableBytes() const noexcept {
    return getCongestionWindow() > inflightBytes_ ? getCongestionWindow() - inflightBytes_ : 0;
}
