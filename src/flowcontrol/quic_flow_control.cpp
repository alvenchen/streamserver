...
namespace quic {
namespace {
folly::Optional<uint64_t> calculateNewWindowUpdate(uint64_t curReadOffset, uint64_t curAdvertisedOffset,
    uint64_t windowSize, const std::chrono::microseconds& srtt, const TransportSettings& transportSettings,
    const folly::Optional<TimePoint>& lastSendTime, const TimePoint& updateTime) {
    ...
    bool enoughTimeElapsed = lastSendTime && updateTime > *lastSendTime &&
        (updateTime - *lastSendTime) > transportSettings.flowControlRttFrequency * srtt;
    ...
    bool enoughWindowElapsed = (curAdvertisedOffset - curReadOffset) * transportSettings.flowControlWindowFrequency < windowSize;
...
inline void incrementWithOverFlowCheck(T& num, T diff) {
    if (num > std::numeric_limits<T>::max() - diff) {
        throw QuicInternalException("flow control state overflow", LocalErrorCode::INTERNAL_ERROR);
    }
    num += diff;
}
...
inline uint64_t calculateMaximumData(const QuicStreamState& stream) {
    return std::max(stream.currentReadOffset + stream.flowControlState.windowSize, stream.flowControlState.advertisedMaxOffset);
}

} // namespace

void maybeIncreaseConnectionFlowControlWindow(QuicConnectionStateBase::ConnectionFlowControlState& flowControlState,
    TimePoint updateTime, std::chrono::microseconds srtt) {
    if (!flowControlState.timeOfLastFlowControlUpdate || srtt == 0us) {
        return;
    }
    //CHECK(updateTime > *flowControlState.timeOfLastFlowControlUpdate);
    if (std::chrono::duration_cast<decltype(srtt)>(updateTime - *flowControlState.timeOfLastFlowControlUpdate) < 2 * srtt) {
        flowControlState.windowSize *= 2;
    }
}

bool maybeSendConnWindowUpdate(QuicConnectionStateBase& conn, TimePoint updateTime) {
    ...
    auto newAdvertisedOffset = calculateNewWindowUpdate(flowControlState.sumCurReadOffset,
        flowControlState.advertisedMaxOffset, flowControlState.windowSize, conn.lossState.srtt,
        conn.transportSettings, flowControlState.timeOfLastFlowControlUpdate, updateTime);
...
bool maybeSendStreamWindowUpdate(QuicStreamState& stream, TimePoint updateTime) {
    ...
    auto newAdvertisedOffset = calculateNewWindowUpdate(stream.currentReadOffset,
        flowControlState.advertisedMaxOffset, flowControlState.windowSize,
        stream.conn.lossState.srtt, stream.conn.transportSettings,
        flowControlState.timeOfLastFlowControlUpdate, updateTime);
    if (newAdvertisedOffset) {
        //VLOG(10) << "Queued flow control update for stream=" << stream.id << " offset=" << *newAdvertisedOffset;
...
void updateFlowControlOnStreamData(QuicStreamState& stream,uint64_t previousMaxOffsetObserved,
    uint64_t bufferEndOffset) {

    if (stream.flowControlState.advertisedMaxOffset < bufferEndOffset) {
        throw QuicTransportException(folly::to<std::string>("Stream flow control violation on stream ", stream.id), TransportErrorCode::FLOW_CONTROL_ERROR);
    }
    auto curMaxOffsetObserved = std::max(previousMaxOffsetObserved, bufferEndOffset);
    ...
    incrementWithOverFlowCheck(connMaxObservedOffset, curMaxOffsetObserved - previousMaxOffsetObserved);
    if (connMaxObservedOffset > connFlowControlState.advertisedMaxOffset) {
        throw QuicTransportException("Connection flow control violation",TransportErrorCode::FLOW_CONTROL_ERROR);
    }
    incrementWithOverFlowCheck(connFlowControlState.sumMaxObservedOffset, curMaxOffsetObserved - previousMaxOffsetObserved);
}

void updateFlowControlOnRead(QuicStreamState& stream, uint64_t lastReadOffset, TimePoint readTime) {
    //DCHECK_GE(stream.currentReadOffset, lastReadOffset);
    auto diff = stream.currentReadOffset - lastReadOffset;
    incrementWithOverFlowCheck(stream.conn.flowControlState.sumCurReadOffset, diff);
    if (maybeSendConnWindowUpdate(stream.conn, readTime)) {
        //VLOG(4) << "Read trigger conn window update "<< " readOffset=" << stream.conn.flowControlState.sumCurReadOffset << " maxOffset=" << stream.conn.flowControlState.advertisedMaxOffset << " window=" << stream.conn.flowControlState.windowSize;
    }
    if (maybeSendStreamWindowUpdate(stream, readTime)) {
        //VLOG(4) << "Read trigger stream window update stream=" << stream.id << " readOffset=" << stream.currentReadOffset << " maxOffset=" << stream.flowControlState.advertisedMaxOffset << " window=" << stream.flowControlState.windowSize;
    }
}

void updateFlowControlOnWriteToSocket(QuicStreamState& stream, uint64_t length) {
    incrementWithOverFlowCheck(stream.conn.flowControlState.sumCurWriteOffset, length);
...
void updateFlowControlOnWriteToStream(QuicStreamState& stream, uint64_t length) {
    incrementWithOverFlowCheck(stream.conn.flowControlState.sumCurStreamBufferLen, length);
}
...
void updateFlowControlOnResetStream(QuicStreamState& stream) {
    decrementWithOverFlowCheck(
        ...
        static_cast<uint64_t>(stream.writeBuffer.chainLength() + stream.writeBufMeta.length));
}
...
void handleStreamWindowUpdate(QuicStreamState& stream, uint64_t maximumData, PacketNum packetNum) {
    if (stream.flowControlState.peerAdvertisedMaxOffset <= maximumData) {
        ...
        if (stream.flowControlState.peerAdvertisedMaxOffset >
            stream.currentWriteOffset + stream.writeBuffer.chainLength() + stream.writeBufMeta.length) {
...
uint64_t getSendStreamFlowControlBytesWire(const QuicStreamState& stream) {
    //DCHECK_GE(stream.flowControlState.peerAdvertisedMaxOffset, stream.nextOffsetToWrite());
    return stream.flowControlState.peerAdvertisedMaxOffset - stream.nextOffsetToWrite();
}
...
uint64_t getSendStreamFlowControlBytesAPI(const QuicStreamState& stream) {
    ...
    auto dataInBuffer = stream.writeBuffer.chainLength() + stream.writeBufMeta.length;
...
uint64_t getSendConnFlowControlBytesWire(const QuicConnectionStateBase& conn) {
    //DCHECK_GE(conn.flowControlState.peerAdvertisedMaxOffset, conn.flowControlState.sumCurWriteOffset);
    return conn.flowControlState.peerAdvertisedMaxOffset - conn.flowControlState.sumCurWriteOffset;
}
...
uint64_t getRecvStreamFlowControlBytes(const QuicStreamState& stream) {
    if (stream.flowControlState.advertisedMaxOffset < stream.currentReadOffset) {
        ...
        // FIN.
        //DCHECK_EQ(stream.currentReadOffset, stream.flowControlState.advertisedMaxOffset + 1);
        return 0;
    }
...
uint64_t getRecvConnFlowControlBytes(const QuicConnectionStateBase& conn) {
    //DCHECK_GE(conn.flowControlState.advertisedMaxOffset, conn.flowControlState.sumCurReadOffset);
    return conn.flowControlState.advertisedMaxOffset - conn.flowControlState.sumCurReadOffset;
}

void onConnWindowUpdateSent(QuicConnectionStateBase& conn, uint64_t maximumDataSent, TimePoint sentTime) {
...
void onStreamWindowUpdateSent(QuicStreamState& stream, uint64_t maximumDataSent, TimePoint sentTime) {
...
void updateFlowControlStateWithSettings(
    ...
    flowControlState.windowSize = transportSettings.advertisedInitialConnectionWindowSize;
    flowControlState.advertisedMaxOffset = transportSettings.advertisedInitialConnectionWindowSize;
}
