...
CoreAllocatorGuard::CoreAllocatorGuard(size_t numStripes, size_t stripe)
  ...
  //CHECK(gCoreAllocatorGuard == nullptr) << "CoreAllocator::Guard cannot be used recursively";
...
void* coreMallocFromGuard(size_t size) {
  //CHECK(gCoreAllocatorGuard != nullptr) << "CoreAllocator::allocator called without an active Guard";
  return coreMalloc(
