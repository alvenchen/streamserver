...
// the fiber's stack.
  ...
  if (FOLLY_UNLIKELY(recordStackUsed_ && !stackFilledWithMagic_)) {
    //CHECK_EQ(reinterpret_cast<intptr_t>(fiberStackLimit_) % sizeof(uint64_t), 0u);
...
[[noreturn]] void Fiber::fiberFunc() {
  ...
  while (true) {
    ...
    if (FOLLY_UNLIKELY(recordStackUsed_)) {
      ...
      //CHECK_LT(newHighWatermark, fiberManager_.options_.stackSize - 64) << "Fiber stack overflow";
    }
