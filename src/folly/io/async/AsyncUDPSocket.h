...
public:
    ...
    public:
        ...
        virtual void onDataAvailable(const folly::SocketAddress& client,
            size_t len, bool truncated, OnDataAvailableParams params) noexcept = 0;
    ...
    static void fromMsg(FOLLY_MAYBE_UNUSED ReadCallback::OnDataAvailableParams& params,
        FOLLY_MAYBE_UNUSED struct msghdr& msg);

    using IOBufFreeFunc = folly::Function<void(std::unique_ptr<folly::IOBuf>&&)>;

    using AdditionalCmsgsFunc = folly::Function<folly::Optional<SocketOptionMap>()>;

    struct WriteOptions {
        WriteOptions() = default;
        WriteOptions(int gsoVal, bool zerocopyVal) : gso(gsoVal), zerocopy(zerocopyVal) {}
        int gso{0};
        bool zerocopy{false};
     ...
     */
    virtual void bind(const folly::SocketAddress& address, BindOptions options = BindOptions());
    ...
    virtual void setNontrivialCmsgs(const SocketNontrivialOptionMap& nontrivialCmsgs);

    virtual void appendCmsgs(const SocketOptionMap& cmsgs);
    virtual void appendNontrivialCmsgs(const SocketNontrivialOptionMap& nontrivialCmsgs);
    virtual void setAdditionalCmsgsFunc(AdditionalCmsgsFunc&& additionalCmsgsFunc) {
     ...
     * of size num
     */
    virtual int writem(Range<SocketAddress const*> addrs, const std::unique_ptr<folly::IOBuf>* bufs, size_t count);
     ...
     */
    virtual ssize_t writeGSO(const folly::SocketAddress& address, const std::unique_ptr<folly::IOBuf>& buf,
        WriteOptions options);

    virtual ssize_t writeChain(const folly::SocketAddress& address,
        std::unique_ptr<folly::IOBuf>&& buf, WriteOptions options);
     ...
     *  verify GSO is supported on this platform by calling getGSO
     */
    virtual int writemGSO(Range<SocketAddress const*> addrs,
        const std::unique_ptr<folly::IOBuf>* bufs, size_t count, const WriteOptions* options);
    ...
    virtual ssize_t writev(const folly::SocketAddress& address,
        const struct iovec* vec, size_t iovec_len, WriteOptions options);
    ...
    virtual int recvmmsg(struct mmsghdr* msgvec,
    ...
    virtual void applyOptions(const SocketOptionMap& options, SocketOptionKey::ApplyPos pos);

    virtual void applyNontrivialOptions(const SocketNontrivialOptionMap& options, SocketOptionKey::ApplyPos pos);
     ...
     */
    virtual void setOverrideNetOpsDispatcher(std::shared_ptr<netops::Dispatcher> dispatcher) {
     ...
     * Override can be used by unit tests to intercept and mock netops:: calls.
     */
    virtual std::shared_ptr<netops::Dispatcher> getOverrideNetOpsDispatcher() const {
...
protected:
    ...
    virtual int sendmmsg(NetworkSocket socket,
        struct mmsghdr* msgvec, unsigned int vlen, int flags) {
        
        return netops_->sendmmsg(socket, msgvec, vlen, flags);
    }

    void fillMsgVec(Range<full_sockaddr_storage*> addrs,
        const std::unique_ptr<folly::IOBuf>* bufs,
        size_t count, struct mmsghdr* msgvec, struct iovec* iov,
        size_t iov_count, const WriteOptions* options, char* control);

    virtual int writeImpl(Range<SocketAddress const*> addrs,
        const std::unique_ptr<folly::IOBuf>* bufs,
        size_t count, struct mmsghdr* msgvec, const WriteOptions* options, char* control);

    virtual ssize_t writevImpl(netops::Msgheader* msg, FOLLY_MAYBE_UNUSED WriteOptions options);
    ...
    uint16_t maxReadsPerEvent_{kDefaultReadsPerEvent}; ///< Max reads per event loop iteration
