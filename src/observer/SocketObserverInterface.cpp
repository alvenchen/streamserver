...
#include "SocketObserverInterface.h"
#include <utility>

namespace quic {

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setOutstandingPackets(const std::deque<OutstandingPacketWrapper>& outstandingPacketsIn) {
    maybeOutstandingPacketsRef = outstandingPacketsIn;
    return std::move(*this);
}

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setWriteCount(const uint64_t writeCountIn) {
    maybeWriteCount = writeCountIn;
    return std::move(*this);
}

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setLastPacketSentTime(const TimePoint& lastPacketSentTimeIn) {
    maybeLastPacketSentTime = lastPacketSentTimeIn;
    return std::move(*this);
}

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setLastPacketSentTime(const folly::Optional<TimePoint>& maybeLastPacketSentTimeIn) {
    maybeLastPacketSentTime = maybeLastPacketSentTimeIn;
    return std::move(*this);
}

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setCwndInBytes(const folly::Optional<uint64_t>& maybeCwndInBytesIn) {
    maybeCwndInBytes = maybeCwndInBytesIn;
    return std::move(*this);
}

SocketObserverInterface::WriteEvent::Builder&&
SocketObserverInterface::WriteEvent::Builder::setWritableBytes(const folly::Optional<uint64_t>& maybeWritableBytesIn) {
...
SocketObserverInterface::AppLimitedEvent::Builder::setOutstandingPackets(const std::deque<OutstandingPacketWrapper>& outstandingPacketsIn) {
    maybeOutstandingPacketsRef = outstandingPacketsIn;
    return std::move(*this);
}

SocketObserverInterface::AppLimitedEvent::Builder&&
SocketObserverInterface::AppLimitedEvent::Builder::setWriteCount(const uint64_t writeCountIn) {
    maybeWriteCount = writeCountIn;
    return std::move(*this);
}

SocketObserverInterface::AppLimitedEvent::Builder&&
SocketObserverInterface::AppLimitedEvent::Builder::setLastPacketSentTime(const TimePoint& lastPacketSentTimeIn) {
    maybeLastPacketSentTime = lastPacketSentTimeIn;
    return std::move(*this);
}
...
void SocketObserverInterface::PacketsWrittenEvent::
    invokeForEachNewOutstandingPacketOrdered(const std::function<void(const OutstandingPacketWrapper&)>& fn) const {
    ...
    {
        const auto startIt = outstandingPackets.end() - static_cast<int64_t>(numAckElicitingPacketsWritten);
        ...
        for (auto it = startIt; it != outstandingPackets.end(); it++) {
            ...
            if (!maybePrevNumAckElicitingPacketsSent) {
                maybePrevNumAckElicitingPacketsSent = it->metadata.totalAckElicitingPacketsSent;
                continue;
            }

            //CHECK_NE(maybePrevNumAckElicitingPacketsSent.value(), it->metadata.totalAckElicitingPacketsSent);
            if (maybePrevNumAckElicitingPacketsSent.value() > it->metadata.totalAckElicitingPacketsSent) {
                needFullWalk = true;
                break;
            }
        }
    ...
    std::vector<std::reference_wrapper<const OutstandingPacketWrapper>> newOutstandingPackets;
...
SocketObserverInterface::PacketsReceivedEvent::ReceivedPacket::ReceivedPacket(
    SocketObserverInterface::PacketsReceivedEvent::ReceivedPacket::
        ...
        : packetReceiveTime(*builderFields.maybePacketReceiveTime.get_pointer()),
        packetNumBytes(*builderFields.maybePacketNumBytes.get_pointer()) {}
...
SocketObserverInterface::PacketsReceivedEvent::Builder&&
SocketObserverInterface::PacketsReceivedEvent::Builder::addReceivedPacket(ReceivedPacket&& packetIn) {
...
SocketObserverInterface::PacketsReceivedEvent::PacketsReceivedEvent(
    SocketObserverInterface::PacketsReceivedEvent::BuilderFields&& builderFields)
    : receiveLoopTime(*builderFields.maybeReceiveLoopTime.get_pointer()),
    numPacketsReceived(*builderFields.maybeNumPacketsReceived.get_pointer()),
    numBytesReceived(*builderFields.maybeNumBytesReceived.get_pointer()),
...
SocketObserverInterface::AcksProcessedEvent::Builder::setAckEvents(const std::vector<AckEvent>& ackEventsIn) {
...
SocketObserverInterface::AcksProcessedEvent::AcksProcessedEvent(
    ...
    : ackEvents(*builderFields.maybeAckEventsRef.get_pointer()) {
}
