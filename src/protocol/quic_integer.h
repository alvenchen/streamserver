 ...
 */
template <typename BufOp>
folly::Expected<size_t, TransportErrorCode> encodeQuicInteger(uint64_t value, BufOp bufop) {
...
public:
    ...
    size_t encode(BufOp appender) const {
        auto size = encodeQuicInteger(_value, std::move(appender));
        if (size.hasError()) {
            fmt::print("Value too large value={:d}\n", _value);

            //throw std::runtime_error(("Value too large ", value_), size.error());
        }
        return size.value();
    }
