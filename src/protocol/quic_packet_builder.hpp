...
public:
    ...
    struct Packet {
        ...
        Packet(RegularQuicWritePacket packetIn, Buf headerIn, Buf bodyIn)
            : packet(std::move(packetIn)), header(std::move(headerIn)), body(std::move(bodyIn)) {}
    };
    ...
    virtual void appendBytes(BufAppender& appender, PacketNum value, uint8_t byteNumber) = 0;
    virtual void appendBytes(BufWriter& writer, PacketNum value, uint8_t byteNumber) = 0;
...
public:
    ...
    InplaceQuicPacketBuilder(BufAccessor& bufAccessor, uint32_t remainingBytes,
        PacketHeader header, PacketNum largestAckedPacketNum, uint8_t frameHint = 8);
    ...
    void appendBytes(BufWriter& writer, PacketNum value, uint8_t byteNumber) override;
...
public:
    ...
    explicit RegularSizeEnforcedPacketBuilder(Packet packet, uint64_t enforcedSize, uint32_t cipherOverhead);
...
public:
    ...
    explicit InplaceSizeEnforcedPacketBuilder(BufAccessor& bufAccessor, Packet packet, uint64_t enforcedSize, uint32_t cipherOverhead);
...
public:
    explicit VersionNegotiationPacketBuilder(ConnectionId sourceConnectionId, ConnectionId destinationConnectionId, const std::vector<QuicVersion>& versions);
...
public:
    PseudoRetryPacketBuilder(uint8_t initialByte, ConnectionId sourceConnectionId,
...
public:
    RetryPacketBuilder(ConnectionId sourceConnectionId, ConnectionId destinationConnectionId,
...
public:
    StatelessResetPacketBuilder(uint16_t maxPacketSize, const StatelessResetToken& resetToken);
...
class PacketBuilderWrapper : public PacketBuilderInterface {
    ...
    PacketBuilderWrapper(PacketBuilderInterface& builderIn, uint32_t writableBytes)
        : builder(builderIn),
        diff(writableBytes > builder.remainingSpaceInPkt() ? 0 : builder.remainingSpaceInPkt() - writableBytes) {}

    [[nodiscard]] uint32_t remainingSpaceInPkt() const override {
        return builder.remainingSpaceInPkt() > diff ? builder.remainingSpaceInPkt() - diff : 0;
    }

    void encodePacketHeader() override {
        //CHECK(false) << "We only support wrapping builder that has already encoded header";
    }
    ...
    void appendBytes(BufAppender& appender, PacketNum value, uint8_t byteNumber) override {
