...
PacketRebuilder::PacketRebuilder(PacketBuilderInterface& regularBuilder,QuicConnectionStateBase& conn)
...
PacketEvent PacketRebuilder::cloneOutstandingPacket(OutstandingPacketWrapper& packet) {
    // Either the packet has never been cloned before, or it's associatedEvent is
    // still in the outstandings.packetEvents set.
    //DCHECK(!packet.associatedEvent || conn_.outstandings.packetEvents.count(*packet.associatedEvent));
    if (!packet.associatedEvent) {
        ...
        ++conn_.outstandings.clonedPacketCount[packet.packet.header.getPacketNumberSpace()];
    }
    return *packet.associatedEvent;
}

folly::Optional<PacketEvent> PacketRebuilder::rebuildFromPacket(OutstandingPacketWrapper& packet) {
    ...
    auto encryptionLevel = protectionTypeToEncryptionLevel(packet.packet.header.getProtectionType());
    ...
    for (auto iter = packet.packet.frames.cbegin();iter != packet.packet.frames.cend();iter++) {
        ...
        switch (frame.type()) {
            ...
            case QuicWriteFrame::Type::WriteCryptoFrame: {
                ...
                auto cryptoWriteResult = writeCryptoFrame(cryptoFrame.offset, *buf, builder_);
                bool ret = cryptoWriteResult.has_value() && cryptoWriteResult->offset == cryptoFrame.offset &&
            ...
            case QuicWriteFrame::Type::MaxStreamDataFrame: {
                const MaxStreamDataFrame& maxStreamDataFrame = *frame.asMaxStreamDataFrame();
                auto stream = conn_.streamManager->getStream(maxStreamDataFrame.streamId);
                if (!stream || !stream->shouldSendFlowControl()) {
                    writeSuccess = true;
                    break;
                }
                shouldWriteWindowUpdate = true;
                auto ret = 0 != writeFrame(generateMaxStreamDataFrame(*stream), builder_);
                windowUpdateWritten |= ret;
                notPureAck |= ret;
                writeSuccess = true;
                break;
            ...
            case QuicWriteFrame::Type::QuicSimpleFrame: {
                ...
                auto updatedSimpleFrame = updateSimpleFrameOnPacketClone(conn_, simpleFrame);
                if (!updatedSimpleFrame) {
                    writeSuccess = true;
                    break;
                }
                bool ret = writeSimpleFrame(std::move(*updatedSimpleFrame), builder_) != 0;
                notPureAck |= ret;
                writeSuccess = ret;
                break;
            }
    ...
    if (shouldRebuildWriteAckFrame) {
        ...
        uint64_t ackDelayExponent = (packetHeader.getHeaderForm() == HeaderForm::Long)
            ? kDefaultAckDelayExponent : conn_.transportSettings.ackDelayExponent;
        const AckState& ackState_ = getAckState(conn_,
        ...
        std::chrono::microseconds ackDelay = (ackingTime > receivedTime
                    ? std::chrono::duration_cast<std::chrono::microseconds>(ackingTime - receivedTime) : 0us);
        ...
        if (!isAckReceiveTimestampsSupported || !peerRequestedTimestampsCount) {
            ...
            writeAckFrameWithReceivedTimestamps(meta, builder_,
                conn_.transportSettings.maybeAckReceiveTimestampsConfigSentToPeer.value(),
...
const BufQueue* PacketRebuilder::cloneCryptoRetransmissionBuffer(const WriteCryptoFrame& frame, const QuicCryptoStream& stream) {
    /**
    ...
    //DCHECK(iter->second->offset == frame.offset) << "WriteCryptoFrame cloning: offset mismatch. " << conn_;
    //DCHECK(iter->second->data.chainLength() == frame.len) << "WriteCryptoFrame cloning: Len mismatch. " << conn_;
    return &(iter->second->data);
}

const BufQueue* PacketRebuilder::cloneRetransmissionBuffer(const WriteStreamFrame& frame, const QuicStreamState* stream) {
    /**
    ...
    auto iter = stream->retransmissionBuffer.find(frame.offset);
    if (iter != stream->retransmissionBuffer.end()) {
        //DCHECK(!frame.len || !iter->second->data.empty()) << "WriteStreamFrame cloning: frame is not empty but StreamBuffer has" << " empty data. " << conn_;
