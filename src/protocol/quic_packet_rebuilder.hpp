...
public:
    PacketRebuilder(PacketBuilderInterface& regularBuilder, QuicConnectionStateBase& conn);

    folly::Optional<PacketEvent> rebuildFromPacket(OutstandingPacketWrapper& packet);
...
private:
    ...
    const BufQueue* cloneCryptoRetransmissionBuffer(const WriteCryptoFrame& frame,
        const QuicCryptoStream& stream);

    const BufQueue* cloneRetransmissionBuffer(const WriteStreamFrame& frame,
