...
folly::Optional<uint64_t> writeStreamFrameHeader(PacketBuilderInterface& builder,StreamId id,
    uint64_t offset, uint64_t writeBufferLen, uint64_t flowControlLen, bool fin,
    folly::Optional<bool> skipLenHint, folly::Optional<StreamGroupId> streamGroupId, bool appendFrame) {
    if (builder.remainingSpaceInPkt() == 0) {
        return folly::none;
    }
    if (writeBufferLen == 0 && !fin) {
        throw QuicInternalException("No data or fin supplied when writing stream.", LocalErrorCode::INTERNAL_ERROR);
    }
    ...
    if (builder.remainingSpaceInPkt() < headerSize) {
        //VLOG(4) << "No space in packet for stream header. stream=" << id << " remaining=" << builder.remainingSpaceInPkt();
        return folly::none;
    }
    ...
    if (skipLenHint) {
        ...
        shouldSkipLengthField = dataLen > 0 && dataLen >= builder.remainingSpaceInPkt() - headerSize;
    }
    ...
    if (!shouldSkipLengthField) {
        if (dataLen <= kOneByteLimit - 1) {
            ...
            throw QuicInternalException("Stream frame length too large.", LocalErrorCode::INTERNAL_ERROR);
        }
    }
    if (dataLenLen > 0) {
        if (dataLen != 0 &&
    ...
    if (appendFrame) {
        builder.appendFrame(WriteStreamFrame(id, offset, dataLen, streamType.hasFin(), false /* fromBufMetaIn */, streamGroupId));
    } else {
...
void writeStreamFrameData(PacketBuilderInterface& builder, const BufQueue& writeBuffer, uint64_t dataLen) {
    if (dataLen > 0) {
        builder.insert(writeBuffer, dataLen);
    }
}

void writeStreamFrameData(PacketBuilderInterface& builder, Buf writeBuffer, uint64_t dataLen) {
    if (dataLen > 0) {
        builder.insert(std::move(writeBuffer), dataLen);    
    }
}

folly::Optional<WriteCryptoFrame> writeCryptoFrame(uint64_t offsetIn, const BufQueue& data, PacketBuilderInterface& builder) {
    ...
    size_t cryptoFrameHeaderSize = intFrameType.getSize() + offsetInteger.getSize() + lengthBytes;

    if (spaceLeftInPkt <= cryptoFrameHeaderSize) {
        //VLOG(3) << "No space left in packet to write cryptoFrame header of size: " << cryptoFrameHeaderSize << ", space left=" << spaceLeftInPkt;
        return folly::none;
    }
    ...
    if (lengthVarInt.getSize() > lengthBytes) {
        throw QuicInternalException(std::string("Length bytes representation"), LocalErrorCode::CODEC_ERROR);
    }
 ...
 */
static size_t fillFrameWithAckBlocks(const AckBlocks& ackBlocks, WriteAckFrame& ackFrame, uint64_t bytesLimit) {
    ...
    for (auto blockItr = ackBlocks.crbegin() + 1; blockItr != ackBlocks.crend(); ++blockItr) {
        ...
        size_t numAdditionalAckBlocksSize = getQuicIntegerSizeThrows(numAdditionalAckBlocks + 1);
        size_t previousNumAckBlocksSize = getQuicIntegerSizeThrows(previousNumAckBlocks);

        size_t additionalSize = gapSize + currBlockLenSize + (numAdditionalAckBlocksSize - previousNumAckBlocksSize);
...
size_t computeSizeUsedByRecvdTimestamps(WriteAckFrame& ackFrame) {
    size_t usedSize = 0;
    for (auto& recvdPacketsTimestampRanges : ackFrame.recvdPacketsTimestampRanges) {
        usedSize += getQuicIntegerSizeThrows(recvdPacketsTimestampRanges.gap);
        usedSize += getQuicIntegerSizeThrows(recvdPacketsTimestampRanges.timestamp_delta_count);
...
static size_t fillPacketReceiveTimestamps(const quic::WriteAckFrameMetaData& ackFrameMetaData,
    WriteAckFrame& ackFrame, uint64_t largestAckedPacketNum, uint64_t spaceLeft,
    uint64_t receiveTimestampsExponent, uint64_t maxRecvTimestampsToSend) {
    ...
    for (auto timestampIntervalsIt = receivedPktNumsIntervalSet.crbegin(); timestampIntervalsIt != receivedPktNumsIntervalSet.crend(); timestampIntervalsIt++) {
        ...
        nextTimestampRangeUsedSpace += getQuicIntegerSizeThrows(nextTimestampRange.gap);
        ...
        while (timestampIt != recvdPacketInfos.crend() &&
            ...
            if (timestampIt == recvdPacketInfos.crbegin()) {
                deltaDuration = (timestampIt->timeStamp > ackFrameMetaData.connTime)
                    ? std::chrono::duration_cast<std::chrono::microseconds>(timestampIt->timeStamp - ackFrameMetaData.connTime)
                    : 0us;
            } else {
                deltaDuration = std::chrono::duration_cast<std::chrono::microseconds>((timestampIt - 1)->timeStamp - timestampIt->timeStamp);
            }
            ...
            if (spaceLeft < (cumUsedSpace + nextTimestampRangeUsedSpace +
                ...
                getQuicIntegerSizeThrows(ackFrame.recvdPacketsTimestampRanges.size() + 1))) {
                outOfSpace = true;
                break;
            }
        ...
        if (nextTimestampRange.deltas.size() > 0) {
            nextTimestampRange.timestamp_delta_count = nextTimestampRange.deltas.size();
            cumUsedSpace += nextTimestampRangeUsedSpace + getQuicIntegerSizeThrows(nextTimestampRange.deltas.size());
...
folly::Optional<WriteAckFrame> writeAckFrameToPacketBuilder(
    ...
    PacketBuilderInterface& builder, FrameType frameType) {
    ...
    auto headerSize = encodedintFrameType.getSize() + largestAckedPacketInt.getSize() + ackDelayInt.getSize() +
    ...
    if (frameType == FrameType::ACK_RECEIVE_TIMESTAMPS) {
        ...
        maybeLastPktTsDelta = (ackState.lastRecvdPacketInfo.value().timeStamp > ackFrameMetaData.connTime
                    ? std::chrono::duration_cast<std::chrono::microseconds>(ackState.lastRecvdPacketInfo.value().timeStamp - ackFrameMetaData.connTime)
                    : 0us);
    ...
    auto numAdditionalAckBlocks = fillFrameWithAckBlocks(ackState.acks, ackFrame, spaceLeft);
    ...
    for (auto it = ackFrame.ackBlocks.cbegin() + 1; it != ackFrame.ackBlocks.cend(); ++it) {
...
folly::Optional<WriteAckFrameResult> writeAckFrame(const quic::WriteAckFrameMetaData& ackFrameMetaData, PacketBuilderInterface& builder, FrameType frameType) {
    uint64_t beginningSpace = builder.remainingSpaceInPkt();
    auto maybeWriteAckFrame = writeAckFrameToPacketBuilder(ackFrameMetaData, builder, frameType);

    if (maybeWriteAckFrame.has_value()) {
        builder.appendFrame(std::move(maybeWriteAckFrame.value()));
        return WriteAckFrameResult(
            beginningSpace - builder.remainingSpaceInPkt(), maybeWriteAckFrame.value(),
...
folly::Optional<WriteAckFrameResult> writeAckFrameWithReceivedTimestamps(
    const quic::WriteAckFrameMetaData& ackFrameMetaData, PacketBuilderInterface& builder,
    const AckReceiveTimestampsConfig& recvTimestampsConfig, uint64_t maxRecvTimestampsToSend) {

    auto beginningSpace = builder.remainingSpaceInPkt();
    auto maybeAckFrame = writeAckFrameToPacketBuilder(ackFrameMetaData, builder, FrameType::ACK_RECEIVE_TIMESTAMPS);
    ...
    if (ackState.lastRecvdPacketInfo) {
        ...
        lastPktTsDelta =
            (ackState.lastRecvdPacketInfo.value().timeStamp > ackFrameMetaData.connTime
                    ? std::chrono::duration_cast<std::chrono::microseconds>(ackState.lastRecvdPacketInfo.value().timeStamp - ackFrameMetaData.connTime)
                    : 0us);
    }
    ...
    ackFrame.maybeLatestRecvdPacketTime = std::chrono::microseconds(lastRecvdPacketTimeInt.getValue());
    ...
    if (spaceLeft > 0) {
        auto largestAckedPacket = ackState.acks.back().end;
        uint8_t receiveTimestampsExponentToUse = recvTimestampsConfig.receiveTimestampsExponent;
        countTimestampRanges = fillPacketReceiveTimestamps(ackFrameMetaData, ackFrame, largestAckedPacket, spaceLeft, receiveTimestampsExponentToUse, maxRecvTimestampsToSend);
        if (countTimestampRanges > 0) {
            QuicInteger timeStampRangeCountInt(ackFrame.recvdPacketsTimestampRanges.size());
            builder.write(timeStampRangeCountInt);
            for (auto& recvdPacketsTimestampRanges : ackFrame.recvdPacketsTimestampRanges) {
                QuicInteger gapInt(recvdPacketsTimestampRanges.gap);
                QuicInteger timestampDeltaCountInt(recvdPacketsTimestampRanges.timestamp_delta_count);
                builder.write(gapInt);
    ...
    auto ackFrameWriteResult = WriteAckFrameResult( beginningSpace - builder.remainingSpaceInPkt(),
        ackFrame, ackFrame.ackBlocks.size(), countTimestampRanges, countTimestamps);

    builder.appendFrame(std::move(ackFrame));
    return ackFrameWriteResult;
}

size_t writeSimpleFrame(QuicSimpleFrame&& frame, PacketBuilderInterface& builder) {
    ...
    switch (frame.type()) {
        case QuicSimpleFrame::Type::StopSendingFrame: {
            ...
            auto stopSendingFrameSize = intFrameType.getSize() + streamId.getSize() + errorSize;
        ...
        case QuicSimpleFrame::Type::NewConnectionIdFrame: {
            ...
            if (packetSpaceCheck(spaceLeft, newConnectionIdFrameSize)) {
                ...
                builder.push(newConnectionIdFrame.connectionId.data(), newConnectionIdFrame.connectionId.size());
                builder.push(newConnectionIdFrame.token.data(), newConnectionIdFrame.token.size());
        ...
        case QuicSimpleFrame::Type::RetireConnectionIdFrame: {
            ...
            auto retireConnectionIdFrameSize = frameType.getSize() + sequence.getSize();
...
size_t writeFrame(QuicWriteFrame&& frame, PacketBuilderInterface& builder) {
    ...
    switch (frame.type()) {
        ...
        case QuicWriteFrame::Type::RstStreamFrame: {
            ...
            size_t errorSize = errorCode.getSize();
            auto rstStreamFrameSize = intFrameType.getSize() + errorSize + streamId.getSize() + offset.getSize();
        ...
        case QuicWriteFrame::Type::MaxStreamDataFrame: {
            ...
            auto maxStreamDataFrameSize = intFrameType.getSize() + streamId.getSize() + maximumData.getSize();
        ...
        case QuicWriteFrame::Type::StreamDataBlockedFrame: {
            ...
            auto blockedFrameSize = intFrameType.getSize() + streamId.getSize() + dataLimit.getSize();
            if (packetSpaceCheck(spaceLeft, blockedFrameSize)) {
                builder.write(intFrameType);
                builder.write(streamId);
                builder.write(dataLimit);
        ...
        case QuicWriteFrame::Type::ConnectionCloseFrame: {
            ...
            const ApplicationErrorCode* isApplicationErrorCode = connectionCloseFrame.errorCode.asApplicationErrorCode();
            ...
            if (isTransportErrorCode) {
                closingFrameType = QuicInteger(static_cast<FrameTypeType>(connectionCloseFrame.closingFrameType));
            }

            QuicInteger errorCode(
                isTransportErrorCode
                    ? static_cast<uint64_t>(TransportErrorCode(*isTransportErrorCode))
                    : static_cast<uint64_t>(ApplicationErrorCode(*isApplicationErrorCode)));

            size_t errorSize = errorCode.getSize();

            auto connCloseFrameSize = intFrameType.getSize() + errorSize +
                (closingFrameType ? closingFrameType.value().getSize() : 0) + reasonLength.getSize() + connectionCloseFrame.reasonPhrase.size();
            if (packetSpaceCheck(spaceLeft, connCloseFrameSize)) {
                ...
                builder.push((const uint8_t*)connectionCloseFrame.reasonPhrase.data(), connectionCloseFrame.reasonPhrase.size());
        ...
        case QuicWriteFrame::Type::DatagramFrame: {
            ...
            auto datagramFrameLength = frameTypeQuicInt.getSize() + datagramFrame.length + datagramLenInt.getSize();
        ...
        default: {
            auto errorStr = folly::to<std::string>("Unknown / unsupported frame type received at ", __func__);
            //VLOG(2) << errorStr;
            throw QuicTransportException(errorStr, TransportErrorCode::FRAME_ENCODING_ERROR);
        }
    }
}
