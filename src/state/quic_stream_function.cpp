...
void writeDataToQuicStream(QuicStreamState& stream, Buf data, bool eof) {
    ...
    //LOG_IF(FATAL, eof && stream.dsrSender) << "Trying to write eof on normal data for DSR stream: " << &stream;
    if (len > 0) {
    ...
    if (eof) {
        auto bufferSize = stream.writeBuffer.front() ? stream.writeBuffer.chainLength() : 0;
...
void writeBufMetaToQuicStream(QuicStreamState& stream, const BufferMeta& data, bool eof) {
    if (data.length > 0) {
        maybeWriteBlockAfterAPIWrite(stream);
    }
    auto realDataLength = stream.currentWriteOffset + stream.writeBuffer.chainLength();
    //CHECK_GT(realDataLength, 0) << "Real data has to be written to a stream before any buffer meta is" << "written to it.";
    if (stream.writeBufMeta.offset == 0) {
        //CHECK(!stream.finalWriteOffset.has_value()) << "Buffer meta cannot be appended to a stream after we have seen EOM " << "in real data";
        stream.writeBufMeta.offset = realDataLength;
    }
    stream.writeBufMeta.length += data.length;
    if (eof) {
        stream.finalWriteOffset = stream.writeBufMeta.offset + stream.writeBufMeta.length;
...
void appendDataToReadBufferCommon(QuicStreamLike& stream, StreamBuffer buffer,
    ...
    if (stream.finalReadOffset && bufferEofOffset && *stream.finalReadOffset != *bufferEofOffset) {
        throw QuicTransportException("Invalid EOF", TransportErrorCode::FINAL_SIZE_ERROR);
    } else if (bufferEofOffset) {
        // Do some consistency checks on the stream.
        if (stream.maxOffsetObserved > *bufferEofOffset) {
            throw QuicTransportException("EOF in middle of stream", TransportErrorCode::FINAL_SIZE_ERROR);
        }
        ...
        if (buffer.offset + buffer.data.chainLength() > *stream.finalReadOffset) {
            throw QuicTransportException("Invalid data after EOF", TransportErrorCode::FINAL_SIZE_ERROR);
        }
    }
    ...
    stream.maxOffsetObserved = std::max(stream.maxOffsetObserved, bufferEndOffset);
    ...
    bool done = false;
    it = std::lower_bound(it, readBuffer.end(), current->offset,
    ...
    for (; it != readBuffer.end() && !done; ++it) {
        ...
        if (current->offset == it->offset && currentEnd == itEnd) {
            ...
            current->offset >= it->offset && current->offset <= itEnd && currentEnd > itEnd) {
    ...
    if (startOverlap && !currentAlreadyInserted) {
        //DCHECK(endOverlap);
        //DCHECK(*startOverlap != readBuffer.end() || *endOverlap == readBuffer.end());
...
void appendDataToReadBuffer(QuicStreamState& stream, StreamBuffer buffer) {
    appendDataToReadBufferCommon(stream, std::move(buffer),
        [&stream](uint64_t previousMaxOffsetObserved, uint64_t bufferEndOffset) {
            updateFlowControlOnStreamData(stream, previousMaxOffsetObserved, bufferEndOffset);
        });
}

void appendDataToReadBuffer(QuicCryptoStream& stream, StreamBuffer buffer) {
    appendDataToReadBufferCommon(stream, std::move(buffer), [](uint64_t, uint64_t) {});
}

std::pair<Buf, bool> readDataInOrderFromReadBuffer(QuicStreamLike& stream, uint64_t amount, bool sinkData) {
    ...
    while ((amount == 0 || remaining != 0) && !stream.readBuffer.empty()) {
        ...
        uint64_t toRead = std::min<uint64_t>(currSize, amount == 0 ? currSize : remaining);
...
std::pair<Buf, bool> readDataFromQuicStream(QuicStreamState& stream, uint64_t amount) {
    auto eof = stream.finalReadOffset && stream.currentReadOffset >= *stream.finalReadOffset;
    if (eof) {
    ...
    eof = stream.finalReadOffset && stream.currentReadOffset == *stream.finalReadOffset;
    if (eof) {
        stream.currentReadOffset += 1;
    }
    stream.conn.streamManager->updateReadableStreams(stream);
...
void peekDataFromQuicStream(QuicStreamState& stream,
    const folly::Function<void(StreamId id, const folly::Range<PeekIterator>&) const>& peekCallback) {
    if (peekCallback) {
        peekCallback(stream.id, folly::Range<PeekIterator>(stream.readBuffer.cbegin(), stream.readBuffer.size()));
    }
}
...
void consumeDataFromQuicStream(QuicStreamState& stream, uint64_t amount) {
    bool eof = stream.finalReadOffset && stream.currentReadOffset >= *stream.finalReadOffset;
    if (eof) {
        if (stream.currentReadOffset == *stream.finalReadOffset) {
...
void appendPendingStreamReset(QuicConnectionStateBase& conn, const QuicStreamState& stream,
    ...
    */
    ...
    conn.pendingEvents.resets.emplace(
        ...
        std::forward_as_tuple(stream.id, errorCode,
            std::min(
...
folly::Optional<uint64_t> getLargestWriteOffsetTxed(const QuicStreamState& stream) {
    ...
    uint64_t currentWriteOffset = std::max<uint64_t>(stream.currentWriteOffset, stream.writeBufMeta.offset);
    return currentWriteOffset - 1;
}

folly::Optional<uint64_t> getLargestDeliverableOffset(const QuicStreamState& stream) {
    ...
    if (stream.ackedIntervals.empty() || stream.ackedIntervals.front().start != 0) {
        return folly::none;
    }
...
QuicCryptoStream* getCryptoStream(QuicCryptoState& cryptoState, EncryptionLevel encryptionLevel) {
...
void processCryptoStreamAck(QuicCryptoStream& cryptoStream, uint64_t offset, uint64_t len) {
    auto ackedBuffer = cryptoStream.retransmissionBuffer.find(offset);
    if (ackedBuffer == cryptoStream.retransmissionBuffer.end() ||
        ackedBuffer->second->offset != offset || ackedBuffer->second->data.chainLength() != len) {
