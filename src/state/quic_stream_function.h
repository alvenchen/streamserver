 ...
 */
std::pair<Buf, bool> readDataFromQuicStream(QuicStreamState& state, uint64_t amount = 0);
 ...
 */
void appendPendingStreamReset(QuicConnectionStateBase& conn, const QuicStreamState& stream, ApplicationErrorCode errorCode);
 ...
 */
std::pair<Buf, bool> readDataInOrderFromReadBuffer(QuicStreamLike& stream, uint64_t amount, bool sinkData = false);
