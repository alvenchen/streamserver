...
#include "../protocol/connection_id_algo.h"
#include "../protocol/quic_read_codec.hpp"
...
#include "../state/pending_path_rate_limiter.h"
#include "../state/quic_stream_group_retransmission_policy.h"
#include "../state/quic_stream_manager.h"
...
struct NetworkData {
    ...
    NetworkData() = default;
    NetworkData(Buf&& buf, const TimePoint& receiveTime) : receiveTimePoint(receiveTime) {
        if (buf) {
    ...
    NetworkData(std::vector<Buf>&& packetBufs, const TimePoint& receiveTime)
        : receiveTimePoint(receiveTime), packets(std::move(packetBufs)), totalData(0) {
...
struct NetworkDataSingle {
    ...
    NetworkDataSingle(std::unique_ptr<folly::IOBuf> buf, const TimePoint& receiveTime)
...
struct OutstandingsInfo {
    ...
    uint64_t numClonedPackets() {
        return clonedPacketCount[PacketNumberSpace::Initial] + clonedPacketCount[PacketNumberSpace::Handshake] +
...
public:
    ...
    void setNotAppLimited() {
        //DCHECK(isAppLimited_);
        isAppLimited_ = false;
        totalAppLimitedTime_ += std::chrono::duration_cast<std::chrono::microseconds>(Clock::now() - appLimitedStartTime_);
    }
    ...
    std::chrono::microseconds getTotalAppLimitedTime() {
        if (isAppLimited_) {
            return std::chrono::duration_cast<std::chrono::microseconds>(
                        Clock::now() - appLimitedStartTime_) + totalAppLimitedTime_;
        }
...
struct Pacer {
     ...
     */
    virtual void refreshPacingRate(uint64_t cwndBytes, std::chrono::microseconds rtt,
     ...
     */
    [[nodiscard]] virtual std::chrono::microseconds getTimeUntilNextWrite(TimePoint currentTime = Clock::now()) const = 0;
...
struct QuicConnectionStateBase : public folly::DelayedDestruction {
     ...
     */
    folly::Function<bool(const folly::Optional<std::string>&, const Buf&) const> earlyDataAppParamsValidator;
    ...
    folly::Optional<AckReceiveTimestampsConfig> maybePeerAckReceiveTimestampsConfig;

    bool peerAdvertisedKnobFrameSupport{false};
    // Retransmission policies map.
    folly::F14FastMap<StreamGroupId, QuicStreamGroupRetransmissionPolicy> retransmissionPolicies;
...
struct AckStateVersion {
    ...
    AckStateVersion(uint64_t initialVersion, uint64_t handshakeVersion, uint64_t appDataVersion);
...
using LossVisitor = std::function<void(QuicConnectionStateBase&, RegularQuicWritePacket&, bool)>;
