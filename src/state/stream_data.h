...
struct QuicStreamLike {
    ...
    folly::F14FastMap<uint64_t, std::unique_ptr<StreamBuffer>> retransmissionBuffer;
    ...
    */
    void insertIntoLossBuffer(std::unique_ptr<StreamBuffer> buf) {
        ...
        auto lossItr = std::upper_bound(lossBuffer.begin(), lossBuffer.end(), buf->offset,
            [](auto offset, const auto& buffer) {
            return offset < buffer.offset; 
        });

        if (!lossBuffer.empty() && lossItr != lossBuffer.begin() &&
            std::prev(lossItr)->offset + std::prev(lossItr)->data.chainLength() == buf->offset) {
    ...
    void removeFromLossBuffer(uint64_t offset, size_t len, bool eof) {
        ...
        for (; lossItr != lossBuffer.end(); lossItr++) {
            ...
            if ((removedStartOffset >= lossStartOffset && removedEndOffset <= lossEndOffset) ||
                (lossStartOffset >= removedStartOffset && lossEndOffset <= removedEndOffset)) {
                size_t amountToSplit = removedStartOffset > lossStartOffset ? removedStartOffset - lossStartOffset : 0;
                ...
                if (splitBuf) {
                    insertIntoLossBuffer(std::make_unique<StreamBuffer>(std::move(splitBuf), lossStartOffset, false));
                }
                return;
            }
        }
...
struct QuicStreamState : public QuicStreamLike {
    ...
    QuicStreamState(QuicConnectionStateBase& connIn, QuicStreamState&& other)
        : QuicStreamLike(std::move(other)), conn(connIn), id(other.id), groupId(other.groupId) {
    ...
    bool inTerminalStates() const {
        bool sendInTerminalState = sendState == StreamSendState::Closed || sendState == StreamSendState::Invalid;

        bool recvInTerminalState = recvState == StreamRecvState::Closed || recvState == StreamRecvState::Invalid;
    ...
    bool hasWritableData() const {
        ...
        if (finalWriteOffset) {
            ...
            // DSR data.
            return writeBufMeta.offset == 0 && currentWriteOffset <= *finalWriteOffset;
        }
        return false;
    }
    ...
    [[nodiscard]] bool hasSentFIN() const {
        ...
        return currentWriteOffset > *finalWriteOffset || writeBufMeta.offset > *finalWriteOffset;
    }
    ...
    bool hasReadableData() const {
        return (readBuffer.size() > 0 && currentReadOffset == readBuffer.front().offset) ||
    ...
    void insertIntoLossBufMeta(WriteBufferMeta bufMeta) {
        auto lossItr = std::upper_bound(lossBufMetas.begin(), lossBufMetas.end(), bufMeta.offset,
        ...
        if (!lossBufMetas.empty() && lossItr != lossBufMetas.begin() &&
            std::prev(lossItr)->offset + std::prev(lossItr)->length == bufMeta.offset) {
